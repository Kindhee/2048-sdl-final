...
void loadBmpImg(SDL_Texture* tabBmpImg[texture_count], SDL_Renderer* renderer){
...
void showTable(SDL_Texture* tabBmpImg[texture_count], SDL_Renderer* renderer, Cell grid[size_gridC][size_gridC], int lost) {
...
int windowGame() {
    ...
    loadBmpImg(tabBmpImg, window.renderer);

    int valid_play;
    int end_of_game = 0;
    int lost = 0;

    while (window.isRunning == 1) {
        ...
        while (end_of_game == 0) {
            ...
            while (SDL_PollEvent(&event))
            {
                ...
                showTable(tabBmpImg, window.renderer, grid.grid, lost);
